...
1. **The canonical schema already forbids it.** `schemas/json/policy/quorum-rules.schema.json` declares `"value": {"type": "integer", "minimum": 0}`. A descriptor carrying `0.75` is invalid, and the runtime rejects it at `RegisterPolicy` with `INVALID_POLICY_DEFINITION`. **No working code depends on the lenient path** — it produces only descriptors that fail later, further away, with a worse error.
   ...
   **Correction (2026-09-10).** As written, the second half of that sentence was **false**: at the time this plan was authored `macp-runtime` accepted a fractional quorum `threshold.value` at `RegisterPolicy` and the bad descriptor only surfaced later as a mis-computed approval bar. It became **true** with Phase 2 of `macp-runtime`'s `plans/backlog-closeout-2026-09.md`, which added the registration-time check (`PolicyRegistry::validate_quorum_threshold`: `threshold.value.fract() != 0.0` → `INVALID_POLICY_DEFINITION: threshold.value 0.75 must be an integer`) and extended it to `mode: "*"` policies, which previously bypassed every quorum constraint. It now holds for both `mode: "macp.mode.quorum.v1"` and the wildcard. The decision this reason supported was correct regardless — reasons 2 and 3 stood on their own — but the claim should not be re-cited against a runtime older than that change.
