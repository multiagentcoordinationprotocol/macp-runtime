...
> **Normative source.** RFC-MACP-0006 §3.8 and RFC-MACP-0001 §9, which describe the paged `ListSessions` contract, together with the `macp-proto` definitions of `page_size`, `page_token`, and `next_page_token` in `proto/macp/v1/core.proto:411-426`. The RFC prose originally predated the proto fields and described an unpaginated listing; that gap was closed upstream in [multiagentcoordinationprotocol/multiagentcoordinationprotocol#77](https://github.com/multiagentcoordinationprotocol/multiagentcoordinationprotocol/pull/77), so the prose and the proto now agree and this runtime follows both.
>
> The RFC leaves traversal strategy open — it requires only a stable order in which a session present for the whole traversal is returned exactly once. The keyset scan documented above satisfies that requirement without being mandated by it. The RFC also states explicitly that the `INVALID_ARGUMENT` returned for an undecodable `page_token` is a transport status rather than a MACP error code, since the codes in the error-codes registry are envelope-scoped and `ListSessions` carries no Envelope.
...
**The initial sync is materialized incrementally.** The runtime takes the registry's session-ID list once, then loads and emits one session at a time, so a subscriber that reads slowly costs one session's worth of memory rather than a copy of the whole registry. The sync still carries every session that was registered when that ID list was taken -- it is never truncated.

**The ID list is a snapshot; the sessions behind it are not.** A session evicted between the ID pass and its turn is silently omitted from the sync. Only terminal (`RESOLVED`/`EXPIRED`/`CANCELLED`) sessions are ever evicted, and only once older than the configured retention (`MACP_SESSION_DISK_RETENTION_SECS`), so an omitted session is always one that is already finished. One consequence is observable: if such a session's terminal event was published after the stream subscribed, the client receives that `Resolved`/`Expired`/`Cancelled` event with no preceding `Created` for it. The same applies to live events at any time -- `session` is unset when the session was evicted before the event reached the subscriber. Treat an event for an unknown session ID as informational and reconcile with `ListSessions`/`GetSession`; do not treat the event stream as an authoritative session inventory.
...
Per-stream state grows with the number of sessions the stream has observed: the runtime remembers one session ID per `Created` event it has emitted, for the lifetime of the stream, to suppress the duplicate `Created` that the initial sync and the live bus would otherwise both report for the same session. That set is not pruned -- a session is registered before its `Created` event is published, so a just-synced ID can still have its `Created` event arrive arbitrarily later and there is no point at which a duplicate becomes impossible. It is released when the client disconnects. Long-lived observers of very high session churn should reconnect periodically.
