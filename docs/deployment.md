...
**This is not data loss.** Reaching the state requires a policy registered before these registration checks existed, and a session in it could only ever have sealed a *negative* outcome with **zero** approvals: the bar was unreachable from the first message, which is exactly the condition RFC-MACP-0011 §4a reads as grounds for a decline. Such a session was already broken. The append-only log is untouched -- the session is simply not loaded -- so correcting the policy and restarting replays it normally.
