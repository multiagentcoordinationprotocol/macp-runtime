...
- **A decline on an empty tally is denied at every schema version**, because a decline needs at least one *decisive* explicit reject and an empty tally has none (RFC-MACP-0007 §6.2). The schema versions differ only in the positive direction.
